	...
	data:extend({
		{
			...
			icons = stacked_icons,
			stack_size = math.floor(data.raw[item_type][item_name].stack_size/stack_size) * stack_size,
			flags = {},
...
function DBL.deferred_stacked_item_updates()
	for stacked_item_name, item_table in pairs(items_to_update) do
		...
		if data.raw[item_type][item_name] then
			...
			data.raw.item[stacked_item_name].stack_size = math.floor(data.raw[item_type][item_name].stack_size/stack_size) * stack_size
